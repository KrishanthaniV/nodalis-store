...
function renderAdminOrders(orders) {
  ...
  tbody.innerHTML = orders.map(function(o) {
    ...
    var addrStr = [addr.street, addr.city, addr.state, addr.zip].filter(Boolean).join(', ') || 'No address';
    var itemsList = (o.items || []).map(function(i) { return i.name + ' x' + i.quantity; }).join(', ');
    return '<tr><td><strong>#' + (o._id?.slice(-8) || 'N/A') + '</strong><br><span style="font-size:.7rem;color:var(--grey);font-family:monospace;">' + (o._id || '') + '</span></td><td>' + (o.customer?.firstName || 'N/A') + ' ' + (o.customer?.lastName || '') + '<br><span style="font-size:.7rem;color:var(--grey);">' + addrStr + '</span></td><td><span title="' + itemsList + '">' + (o.items?.length || 0) + ' items</span></td><td>LKR ' + (o.total || 0).toLocaleString() + '</td><td><span class="status-badge ' + (statusMap[o.status] || 'status-pending') + '">' + (o.status || 'pending') + '</span></td><td>' + (o.createdAt ? new Date(o.createdAt).toLocaleDateString() : 'N/A') + '</td><td><select onchange="updateOrderStatus(\'' + o._id + '\', this.value)" style="padding:0.3rem 0.5rem;font-size:0.8rem;border:1px solid var(--cream-dark);"><option value="pending"' + (o.status === 'pending' ? ' selected' : '') + '>Pending</option><option value="shipped"' + (o.status === 'shipped' ? ' selected' : '') + '>Shipped</option><option value="delivered"' + (o.status === 'delivered' ? ' selected' : '') + '>Delivered</option><option value="cancelled"' + (o.status === 'cancelled' ? ' selected' : '') + '>Cancelled</option></select></td></tr>';
  }).join('');
}
